"""ARC dataloader for Haystack framework.

This module provides a Haystack-specific dataloader for the ARC (AI2 Reasoning
Challenge) dataset. It extends the base loading functionality with Haystack
...
Processing Pipeline:
    ...
    5. Apply recursive document splitting for chunking
...
class ARCDataloader:
    """Dataloader for ARC dataset with Haystack integration.

    This dataloader loads the ARC science QA dataset and processes it into
    Haystack Document objects suitable for indexing and retrieval. It handles
    ...
    def load_data(self) -> list[dict[str, Any]]:
        ...
        if self.data is None:
            ...
            for row in self.dataset:
                ...
                # Format question with multiple-choice options inline
                # ARC uses A/B/C/D labels paired with choice text
                formatted_question = question
                for label, text in zip(choices["label"], choices["text"]):
                    formatted_question += f"\n{label}: {text}"

                # Extract the correct answer text by matching answerKey to label
                ...
                # Extract context passages as evidence documents
                # ctxs contains Wikipedia passages retrieved for each question
                contexts = row["ctxs"]
        ...
        # This ensures documents fit within embedding model context windows
