"""Unit tests for Qdrant query enhancement pipelines."""
...
class TestQdrantQueryEnhancement:
    """Test class for Qdrant query enhancement feature."""

    @pytest.fixture
    def qdrant_config(self) -> dict[str, Any]:
        """Sample Qdrant configuration."""
        return {
            "qdrant": {
    ...
    @patch("vectordb.haystack.query_enhancement.indexing.qdrant.DataloaderCatalog")
    @patch("vectordb.haystack.query_enhancement.indexing.qdrant.QdrantVectorDB")
    @patch(
        "vectordb.haystack.query_enhancement.indexing.qdrant.create_document_embedder"
    )
    @patch("vectordb.haystack.query_enhancement.indexing.qdrant.validate_config")
    @patch("vectordb.haystack.query_enhancement.indexing.qdrant.load_config")
    def test_indexing_pipeline_initialization(
        ...
        pipeline = QdrantQueryEnhancementIndexingPipeline("/tmp/fake_config.yaml")

        mock_load_config.assert_called_once_with("/tmp/fake_config.yaml")
        mock_validate.assert_called_once_with(qdrant_config)
        mock_dataloader_catalog.create.assert_called_once()
        mock_embedder_factory.assert_called_once_with(qdrant_config)
    ...
    @patch("vectordb.haystack.query_enhancement.search.qdrant.create_groq_generator")
    @patch("vectordb.haystack.query_enhancement.search.qdrant.QueryEnhancer")
    @patch("vectordb.haystack.query_enhancement.search.qdrant.QdrantVectorDB")
    @patch("vectordb.haystack.query_enhancement.search.qdrant.create_text_embedder")
    @patch("vectordb.haystack.query_enhancement.search.qdrant.validate_config")
    @patch("vectordb.haystack.query_enhancement.search.qdrant.load_config")
    def test_search_pipeline_initialization(
        ...
        pipeline = QdrantQueryEnhancementSearchPipeline("/tmp/fake_config.yaml")
    ...
    def test_search_pipeline_run_without_rag(
        ...
        config_no_rag = qdrant_config.copy()
    ...
    def test_search_pipeline_hyde_enhancement(
        ...
        hyde_config = qdrant_config.copy()
    ...
    def test_search_pipeline_step_back_enhancement(
        ...
        step_back_config = qdrant_config.copy()
