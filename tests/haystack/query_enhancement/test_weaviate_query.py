"""Unit tests for Weaviate query enhancement pipelines."""
...
class TestWeaviateQueryEnhancement:
    """Test class for Weaviate query enhancement feature."""

    @pytest.fixture
    def weaviate_config(self) -> dict[str, Any]:
        """Sample Weaviate configuration for testing."""
        return {
            "weaviate": {
    ...
    @pytest.fixture
    def weaviate_config_with_rag(
        self, weaviate_config: dict[str, Any]
    ) -> dict[str, Any]:
        """Configuration with RAG enabled."""
        config = weaviate_config.copy()
        config["rag"] = {
            "enabled": True,
            "provider": "groq",
    ...
    @patch("vectordb.haystack.query_enhancement.indexing.weaviate.DataloaderCatalog")
    @patch("vectordb.haystack.query_enhancement.indexing.weaviate.WeaviateVectorDB")
    @patch(
        "vectordb.haystack.query_enhancement.indexing.weaviate.create_document_embedder"
    )
    @patch("vectordb.haystack.query_enhancement.indexing.weaviate.validate_config")
    @patch("vectordb.haystack.query_enhancement.indexing.weaviate.load_config")
    def test_indexing_pipeline_initialization(
        ...
        pipeline = WeaviateQueryEnhancementIndexingPipeline("/tmp/fake_config.yaml")

        mock_load_config.assert_called_once_with("/tmp/fake_config.yaml")
        mock_validate.assert_called_once_with(weaviate_config)
        mock_dataloader_catalog.create.assert_called_once()
        mock_embedder_factory.assert_called_once_with(weaviate_config)
    ...
    @patch("vectordb.haystack.query_enhancement.search.weaviate.create_groq_generator")
    @patch("vectordb.haystack.query_enhancement.search.weaviate.QueryEnhancer")
    @patch("vectordb.haystack.query_enhancement.search.weaviate.WeaviateVectorDB")
    @patch("vectordb.haystack.query_enhancement.search.weaviate.create_text_embedder")
    @patch("vectordb.haystack.query_enhancement.search.weaviate.validate_config")
    @patch("vectordb.haystack.query_enhancement.search.weaviate.load_config")
    def test_search_pipeline_initialization(
        ...
        pipeline = WeaviateQueryEnhancementSearchPipeline("/tmp/fake_config.yaml")
    ...
    def test_search_pipeline_with_rag(
        ...
        mock_query_enhancer_cls: MagicMock,
        mock_groq: MagicMock,
        weaviate_config_with_rag: dict[str, Any],
        mock_documents: list[Document],
    ) -> None:
        """Test search pipeline with RAG generation enabled."""
        mock_load_config.return_value = weaviate_config_with_rag
    ...
    def test_search_pipeline_hyde_enhancement(
        ...
        hyde_config = weaviate_config.copy()
    ...
    def test_search_pipeline_step_back_enhancement(
        ...
        step_back_config = weaviate_config.copy()
    ...
    def test_search_pipeline_default_enhancement(
        ...
        default_config = weaviate_config.copy()
    ...
    def test_search_pipeline_handles_search_failure(
        ...
        config_no_rag = weaviate_config.copy()
